...
top_reviewer = users_dataFrame.select("user_id", "name", "elite").orderBy("elite", ascending=False)

top_reviewers = top_business.join(top_reviews, on=['business_id'], how='inner').orderBy("city", ascending=False).\
                     join(top_reviewer, on=['user_id'], how='inner').orderBy("elite", ascending=False)
