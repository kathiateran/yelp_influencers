...
top_reviewer = users_dataFrame.select("user_id", "name", "yelping_since").orderBy("yelping_since", ascending=True)

top_reviewers = top_business.join(top_reviews, on=['business_id'], how='inner').orderBy("categories", ascending=False).\
                     join(top_reviewer, on=['user_id'], how='inner').orderBy("yelping_since", ascending=True)
